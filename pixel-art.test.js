...
assert.deepEqual(PixelArt.getReferenceSpriteCrop('sword'), { x: 72, y: 48, width: 270, height: 250 });
assert.deepEqual(PixelArt.getReferenceSpriteCrop('furnace-lord'), { x: 15, y: 585, width: 345, height: 375 });
assert.equal(PixelArt.getReferenceSpriteCrop('unknown'), null);

const spriteCtx = recordingContext();
spriteCtx.drawImage = (...args) => spriteCtx.operations.push(['drawImage', ...args]);
const fakeSheet = { complete: true };
PixelArt.setReferenceSpriteSheet(fakeSheet);
assert.equal(PixelArt.drawReferenceSprite(spriteCtx, 'staff', { x: 80, y: 250, scale: 2 }), true);
assert.equal(spriteCtx.operations.some(operation => operation[0] === 'drawImage'), true);
PixelArt.setReferenceSpriteSheet(null);
...
const outlineCtx = recordingContext();
PixelArt.drawHero(outlineCtx, { x: 80, y: 250, scale: 2, weapon: 'staff', frame: 0, pose: 'idle' });
assert.ok(outlineCtx.operations.some(operation => operation[0] === 'stroke' && operation.includes('#090b0c')));
const glowCtx = recordingContext();
PixelArt.drawSpriteGlow(glowCtx, { x: 80, y: 250, color: '#66d3c2', radius: 42 });
assert.ok(glowCtx.operations.some(operation => operation.includes('#66d3c2')));

const heroStyleColors = ['#496f5d', '#244c73', '#1f6570'];
for (const [index, weapon] of ['sword', 'staff', 'crossbow'].entries()) {
  const ctx = recordingContext();
  PixelArt.drawHero(ctx, { x: 80, y: 250, scale: 2, weapon, frame: 0, pose: 'idle' });
  assert.ok(ctx.operations.some(operation => operation.includes(heroStyleColors[index])));
}
...
const weaponAttackSignatures = ['sword', 'staff', 'crossbow'].map(weapon => {
  const ctx = recordingContext();
  PixelArt.drawAttackMotion(ctx, { x: 80, y: 250, scale: 2, weapon, attackProgress: 0.5, attackTarget: { x: 240, y: 125 } });
  assert.ok(ctx.operations.length > 0);
  return JSON.stringify(ctx.operations);
});
assert.equal(new Set(weaponAttackSignatures).size, 3);
const swordWindup = recordingContext();
const swordFollowThrough = recordingContext();
PixelArt.drawHero(swordWindup, { x: 80, y: 250, scale: 2, weapon: 'sword', frame: 0, pose: 'attack' });
PixelArt.drawHero(swordFollowThrough, { x: 80, y: 250, scale: 2, weapon: 'sword', frame: 2, pose: 'attack' });
assert.notDeepEqual(swordWindup.operations, swordFollowThrough.operations);
...
const bossShapes = bossKinds.map(kind => {
  const ctx = recordingContext();
...
assert.equal(new Set(bossShapes).size, bossKinds.length);
